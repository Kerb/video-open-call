...
Самая частая причина — **порт не совпадает**.

Railway задаёт переменную `PORT` автоматически. Наше приложение читает её: `process.env.PORT || 3000`.

Если всё равно 502:

1. Зайди в дашборд → **Variables** — проверь, что `PORT` установлен
2. Если `PORT` не задан — создай переменную `PORT` = `8080` (Railway иногда использует 8080 как внутренний порт)
3. Нажми **Redeploy**

Или укажи порт явно в настройках сервиса: **Settings → Networking → Port**.
...
- `.git/` — игнорируется
- `progress.md`, `videocall-spec.md`, `deploy.md`, `README.md` — можно не пушить,
  но они не влияют на работу
- `docs/` — внутренняя документация (спецификации, планы)
